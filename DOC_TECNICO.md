...
| **Tiempo real (SSE)** | `/sse/orders` — stream de pedidos |
...
```bash
# Backend
cd servidor2 && npm run dev
# Frontend
cd amajoh-pos && npm run dev
```
...
- No hay CI/CD activo.  
- Configuración duplicada (`src/config` vs `src/api/config`).

## 12. Mantenimiento
- Modularidad alta en frontend y backend.  
- Recomendado: unificar configuración, añadir validación de datos y logs estructurados.  
